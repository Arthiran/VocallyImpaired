...
bool FSpeechRecognitionWorker::EnableLanguageModel(FString _languageModel)
{
	#if PLATFORM_WINDOWS
		const char* name = TCHAR_TO_ANSI(*_languageModel);
		...
		JNIEnv* Env = FAndroidApplication::GetJavaEnv();
		if (Env != NULL) {
			jstring langModel = Env->NewStringUTF(languageModel.c_str());
...
void FSpeechRecognitionWorker::AddWords(TArray<FRecognitionPhrase> _keywords) {
	this->keywords.clear();
	for (auto It = _keywords.CreateConstIterator(); It; ++It)
	{
...
int16 FSpeechRecognitionWorker::GetAverageVolume(float startTime, float endTime) {
	...
	if (peakSum > 0)
	{
		_currentVolume = peakSum / peaks.size();
	}

	return _currentVolume;
}

void FSpeechRecognitionWorker::SetLanguage(ESpeechRecognitionLanguage _language) {
	...
	while (StopTaskCounter.GetValue() == 0) {
		...
		if (initRequired) {
			...
			if (detectionMode == ESpeechRecognitionMode::VE_KEYWORD) {
				...
				float64* tolerances;
				tolerances = (float64*)malloc(keywords.size() * sizeof(float64));
				int i = 0;
